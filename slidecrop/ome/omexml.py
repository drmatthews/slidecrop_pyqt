...
# This has been borrowed from the python-bioformats package
...
class OMEXML(object):
    ...
    class Image(object):
        ...
        def set_Description(self, text):
            make_text_node(self.node, self.ns["ome"], "Description", text)
    ...
    class TiffData(object):
        ...
        @property
        def UUID(self):
            node = self.node.find(qn(self.ns['ome'], "UUID"))
            if node is None:
                node = ElementTree.SubElement(
                    self.node, qn(self.ns['ome'], "UUID"))
            return OMEXML.UUID(node)
    ...
    class UUID(object):          
        ...
        def set_FileName(self, value):
            self.node.set("FileName", str(value))

        FileName = property(get_FileName, set_FileName)

        def get_UUID_text(self):
            return self.node.text

        def set_UUID_text(self, value):
            self.node.text = str(value)

        UUID_text = property(get_UUID_text, set_UUID_text)
    ...
    class Pixels(object):
        ...
        def set_tiffdata_count(self, value):
            ...
            if tiffdata_count > value:
                tds = self.node.findall(qn(self.ns['ome'], "TiffData"))
                for td in tds[value:]:
                    self.node.remove(td)
            else:
                for _ in range(tiffdata_count, value):
                    new_tiffdata = OMEXML.Plane(
